...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('guard_name'), 'has-success': fields.guard_name && fields.guard_name.valid }">
    ...
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.guard_name" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('guard_name'), 'form-control-success': fields.guard_name && fields.guard_name.valid}" id="guard_name"
            name="guard_name" placeholder="{{ trans('admin.role.columns.guard_name') }}">
        <div v-if="errors.has('guard_name')" class="form-control-feedback form-text" v-cloak>
            @{{ errors.first('guard_name') }}</div>
    </div>
</div>

<div class="form-group row align-items-start" :class="{'has-danger': errors.has('permissions'), 'has-success': fields.permissions && fields.permissions.valid }">
    <label for="permissions" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.permission.title') }}</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <multiselect v-model="form.permissions" placeholder="{{ trans('brackets/admin-ui::admin.forms.select_options') }}" label="name" track-by="id" :close-on-select="false" :clear-on-select="false" :searchable="true" :options="{{ $permissions->toJson() }}"
